...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let fileBuffer = null, fileName = 'file', fileType = 'application/octet-stream';
    ...
    if (isPDF) {
      ...
      const uploadRes = await new Promise((resolve, reject) => {
        const options = {
          ...
          path: `/storage/v1/object/print-files/${filePath}`,
          method: 'POST',
          headers: {
      ...
      if (uploadRes.status === 200 || uploadRes.status === 201) {
        const publicUrl = `${SURL}/storage/v1/object/public/print-files/${filePath}`;
...
async function uploadToCloudinary(fileBuffer, fileName, fileType, resourceType, res) {
  const timestamp = Math.round(Date.now() / 1000);
  const paramsToSign = `timestamp=${timestamp}`;
  const signature = crypto.createHash('sha1').update(paramsToSign + 'GbxJF-HH8YZxUvOGPBW8Z_s9p_c').digest('hex');

  const formData = buildMultipart({
    file: { buffer: fileBuffer, filename: fileName, contentType: fileType },
    api_key: '422946848896529',
  ...
  const cloudRes = await new Promise((resolve, reject) => {
    const options = {
      ...
      path: `/v1_1/b2wkfr8r/${resourceType}/upload`,
      method: 'POST',
      headers: { 'Content-Type': formData.contentType, 'Content-Length': formData.body.length }
    };
    const r = https.request(options, (resp) => {
      let data = '';
      resp.on('data', chunk => data += chunk);
...
function buildMultipart(fields) {
  ...
  return { contentType: `multipart/form-data; boundary=${boundary}`, body: Buffer.concat(buffers) };
}
